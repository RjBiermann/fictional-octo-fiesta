// vendored for CI from the maintainer local install — upstream: https://github.com/juliusbrussee/caveman (@caveman-ai/cli generates this file via its pi hook; CLI hooks do not target pi, so the artifact ships vendored; fail-open if caveman CLI absent)
// caveman:native-pi — GENERATED by `caveman enable pi`.
process.env.CAVEMAN_PI_HOOK_CMD ??= "[\"/home/yashas/.nvm/versions/node/v24.20.0/bin/caveman\"]";
...
var UPSTREAM_HOSTS_BY_PROVIDER = {
  anthropic: "api.anthropic.com",
  openai: "api.openai.com",
  google: "generativelanguage.googleapis.com",
  "opencode-go": "opencode.ai"
};
function upstreamHostFor(provider) {
  return provider ? UPSTREAM_HOSTS_BY_PROVIDER[provider] : void 0;
}
function hostOf(url) {
...
var COMPAT_NAME_RE = /^[a-z0-9][a-z0-9._-]{0,63}$/;
function compatUpstreamFor(provider, compatUpstreams) {
  if (!compatUpstreams || !COMPAT_NAME_RE.test(provider)) return void 0;
  const url = compatUpstreams[provider];
  return typeof url === "string" ? url : void 0;
}
var COMPAT_SUFFIX_BY_API = {
  "anthropic-messages": "",
  "openai-completions": "/v1",
  "openai-responses": "/v1"
};
function routeForApi(gateway, api, provider, originalBaseUrl, compatUpstreams) {
  if (provider) {
    const mount = compatUpstreamFor(provider, compatUpstreams);
    if (mount !== void 0) {
      const host = hostOf(mount);
      if (!host) return void 0;
      if (hostOf(originalBaseUrl) !== host) return void 0;
      const suffix = api ? COMPAT_SUFFIX_BY_API[api] : void 0;
      return suffix === void 0 ? void 0 : joinUrl(gateway, `/w/pi/compat/${provider}${suffix}`);
    }
    const expected = UPSTREAM_HOSTS_BY_PROVIDER[provider];
    if (!expected) return void 0;
    if (originalBaseUrl !== void 0 && hostOf(originalBaseUrl) !== expected) return void 0;
  }
  const table = (provider ? ROUTES_BY_PROVIDER_API[provider] : void 0) ?? ROUTES_BY_API;
  const path = api ? table[api] : void 0;
...
var ProviderRouter = class {
  ...
  async openGate(gateway, ctx, compatUpstreams) {
  ...
  async apply(model, ctx) {
    ...
    if (!route || oauth) {
      ...
      if (!this.warnedModels.has(key)) {
        ...
        const expected = mount !== void 0 ? hostOf(mount) : upstreamHostFor(model.provider);
        const reason = oauth ? "OAuth/subscription credentials are not routed" : expected === void 0 ? `no compat mount named "${model.provider}" in the local proxy; add compat.${model.provider}.base_url to caveman.yaml to route it` : hostOf(original) !== expected ? `provider endpoint ${hostOf(original) ?? original} is not ${expected}` : `unsupported provider/API "${model.provider}/${model.api}"`;
...
function compatUpstreamsOf(raw) {
