...
**Wired as of devloop v0.3.28** (v0.3.27 → v0.3.28: `--version` derives the released version from package metadata instead of a stale hardcoded literal; no command, config, or permission change. v0.3.26 → v0.3.27: internal refactor — `core.py` split at the build seam into `build.py`, shared queue selection policy; no command, config, or permission change. v0.3.25 → v0.3.26: command args accept `#NNN` — `/repair #486` no longer dies on a `ValueError`; no config or permission change. Prior v0.3.24 → v0.3.25: ghost-diff guard — an empty/404 PR diff aborts the round loudly with a comment instead of degrading to a bare LGTM/"nothing to fix"; no config change. Prior v0.3.19 → v0.3.24: `/repair` — on-demand Review re-derives findings against the current diff, Repair acts on them in a fresh checkout; `repair_rounds=0` disables it; failed-round comments carry the agent-output tail. The pin in `.github/workflows/devloop.yml` is the adopter-facing change): the `/retry` `/review` `/repair` command vocabulary,
